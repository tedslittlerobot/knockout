...
const source = computed(() => rosters.rosters.find(item => item.id === props.route.rosterId)!)

const name = ref<string>(source.value.name)
const nameElement = ref<HTMLElement|null>(null)
const contenders = ref<{ id: string, name: string, avoid: string }[]>(source.value.contenders.map(({ id, name, avoid }) => ({ id, name, avoid: avoid ? avoid.source : '' })))
